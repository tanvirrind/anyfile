...
export const AdminAuthGuard: React.FC<AdminAuthGuardProps> = ({
  ...
  // Check lockout on mount and tick down timer if active
  useEffect(() => {
    const checkLock = () => {
  ...
  const handleLogin = (e?: React.FormEvent) => {
    ...
    if (!identifier.trim() || !passkey.trim()) {
      setErrorMessage('Please enter both administrative identifier and security passkey.');
      return;
    }
    ...
    setTimeout(() => {
      const result = loginAdmin(identifier, passkey, rememberMe);
      setIsLoading(false);
  ...
  const handleDemoAutofill = () => {
    setIdentifier('admin@anyfilex.com');
    setPasskey('AnyFileX-Admin-2026!');
    setErrorMessage(null);
  };
        ...
        {lockoutRemaining !== null && (
          <div className="p-3.5 rounded-xl bg-rose-500/15 border border-rose-500/40 text-rose-300 text-xs flex items-start gap-2.5 animate-in fade-in">
            ...
            <div>
              ...
              <span>Too many incorrect attempts. Authentication is temporarily locked for {lockoutRemaining} seconds.</span>
            </div>
          </div>
        )}
          ...
          <div className="space-y-1.5 text-left">
            ...
            <div className="relative">
              ...
              <input
                ...
                placeholder="admin@anyfilex.com"
                disabled={isLoading || lockoutRemaining !== null}
                className="w-full pl-9 pr-3 py-2.5 bg-slate-900 border border-slate-700 rounded-xl text-sm text-white placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all disabled:opacity-50"
