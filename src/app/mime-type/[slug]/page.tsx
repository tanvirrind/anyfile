...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> { const { slug } = await params; const item = findMime(slug); return item ? { title: `${item.mimeType} MIME Type – AnyFileX`, description: item.description, alternates: { canonical: `https://www.anyfilex.com/mime-type/${slug.toLowerCase()}` } } : { title: 'MIME Type Not Found', robots: { index: false, follow: false } }; }
